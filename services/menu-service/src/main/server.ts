...
app.listen(env.port, () => {
  console.log(`[menu-service] ouvindo na porta ${env.port} (${env.nodeEnv})`);
});
...
async function initSchemaWithRetry(attempt = 1): Promise<void> {
  try {
    await ensureSchema();
    console.log('[menu-service] schema do cardápio pronto.');
  } catch {
    const waitMs = Math.min(30000, attempt * 3000);
    console.error(
      `[menu-service] banco indisponível para migração (tentativa ${attempt}). ` +
